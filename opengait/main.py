...
from utils import config_loader, get_ddp_module, init_seeds, params_count, get_msg_mgr,backbone_params_count,backbone_params_count_v2,backbone_params_count_v1
...
parser.add_argument('--phase', default='train',
                    choices=['train', 'test','visualize'], help="choose train or test phase")
...
def run_model(cfgs, phase):
    ...
    model = get_ddp_module(model, cfgs['trainer_cfg']['find_unused_parameters'])
    #msg_mgr.log_info(params_count(model))
    msg_mgr.log_info(backbone_params_count(model))
    msg_mgr.log_info("Model Initialization Finished!")
    ...
    if phase == 'train':
        ...
        Model.run_visualization(model)  # <--- 新增的调用
