...
from data.transform2 import get_transform
from data.collate_fn import CollateFn
#from data.datasets.dataset_m import DataSet
...
class BaseModel(MetaModel, nn.Module):
    ...
    def inputs_pretreament(self, inputs):
        ...
        if seqL is not None:
            ...
            ipts = seqs
        # print("\n[DEBUG] Checking Input Data Stats:")
        # for idx, item in enumerate(ipts):
        #     print(f"  > Input[{idx}] Shape: {item.shape}")
        #     print(
        #         f"  > Input[{idx}] Range: Min={item.min().item():.4f}, Max={item.max().item():.4f}, Mean={item.mean().item():.4f}")
        #
        #     # 简单判断它是骨骼还是剪影
        #     if item.shape[-1] == 3:
        #         print("    (Likely Skeleton Data)")
        #         if item.max() > 2.0:
        #             print("    [WARNING] 骨骼数据似乎未归一化！数值超过了 2.0，这可能是导致 16% 准确率的原因。")
        #     elif len(item.shape) >= 4:
        #         print("    (Likely Silhouette Data)")
        #         if item.shape[-1] != 44:
        #             print(f"    [WARNING] 剪影宽度为 {item.shape[-1]}，而 CASIA-B 通常为 44。请检查 resize。")
        # print("==================================================\n")
        del seqs
